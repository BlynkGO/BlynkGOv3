...
void GRAPHIC::design(){
  gauge_temp.size(180,180);
  gauge_temp.color(TFT_YELLOW,TFT_RED);
  gauge_temp.range(0,60);
  gauge_temp.decimal(2);
  gauge_temp.align(ALIGN_CENTER, -gauge_temp.width()/2-15, 20);

  lb_temp_info = "อุณหภูมิ";
  lb_temp_info.font(prasanmit_25, TFT_SILVER);
  lb_temp_info.align(gauge_temp, ALIGN_IN_BOTTOM, 0,-15);

  gauge_humid.size(gauge_temp);
  gauge_humid.color(TFT_CYAN, TFT_BLUE);
  gauge_humid.range(0,100);
  gauge_humid.decimal(2);
  gauge_humid.align(gauge_temp, ALIGN_RIGHT,30);

  lb_humid_info = "ความชื้น";
  lb_humid_info.font(prasanmit_25, TFT_SILVER);
...
void GRAPHIC::BeeNeXT_Disconnected(){
  lb_beenext_connection.font(prasanmit_25,TFT_RED);
