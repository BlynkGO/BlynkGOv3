...
void PowerMeter::design(GWidget*parent){
  //--------------------------------
  // ส่วน รูปมิเตอร์
  powermeter_mitsu.parent(parent);
  powermeter_mitsu.align(ALIGN_LEFT,50);
  ...
  lb_power_val.align( ALIGN_TOP_RIGHT, -50, lb_power_info.bottomY()-12);
  ...
  lb_current_val.align( ALIGN_TOP_RIGHT, -50, lb_current_info.bottomY()-12);
  lb_current_val.linkStyle(lb_power_val);
  lb_current_val = String(current_val, 1);

  PowerMeter::value(432.7);

}

void PowerMeter::value(float val) {
