...
void setup() {
  ...
  chart.size(150,100);      // กำหนดขนาดของ chart แผนภูมิกราฟ
...
//  chart.x_tick_texts("1\n2\n3\n4\n5\n6\n7", 2, LV_CHART_AXIS_DRAW_LAST_TICK);
  ...
  //--------------------------------
  timer.setInterval(200,[](){               // ตั้งเวลาทำงานทุกๆ 100 มิลลิวินาที
    static int x=-25;
    x = x+25;                               // ค่า x ทางสมการ math
    int y = 100*sin(radians(x));            // ค่า y ทางสมการ math
    int y_chart = map(y,-100,100, 0,100);   // ค่า y สำหรับใช้กำหนดให้ chart เพื่อวาดกราฟ
    chart.addPoint(serie1, y_chart );       // เพิ่มข้อมูลเข้าไปใน กราฟแรก
