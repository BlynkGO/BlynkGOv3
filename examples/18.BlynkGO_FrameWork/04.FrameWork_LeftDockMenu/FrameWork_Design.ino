...
GRect rect_body;      // rect_body มี page_left_dockmenu และ screens[i] ต่างๆ เป็นเลเยอร์ลูก
  GPage page_left_dockmenu(rect_body);

  GAnimation  anim_page_left_dockmenu;
  GTimer      timer_anim;
...
void framework_design(){
  ...
  rect_titlebar.height(lb_title);
    ...
    btn_dock.align(ALIGN_TOP_LEFT, 5,0);
    btn_dock.onClicked([](GWidget*w){
      page_left_dockmenu_init_show_or_hide();
    });
...
#endif
    lb_date.align(btn_dock, ALIGN_RIGHT, 5);
  #if BLYNKGO_USE_BLYNK
  ...
  rect_body.onGesture([](GWidget*w){
    ...
    switch(gesture_dir){
      case GESTURE_DIR_RIGHT:
        if( page_left_dockmenu.posX() == -page_left_dockmenu.width() ){
          anim_page_left_dockmenu_show();
        }
        break;
      case GESTURE_DIR_LEFT:
        if( page_left_dockmenu.posX() == 0 ){
          anim_page_left_dockmenu_hide();
        }
        break;
    }
  });
  ...
  page_left_dockmenu.size( 84, rect_body.height() );
  page_left_dockmenu.corner_radius(0);
  page_left_dockmenu.layout(LAYOUT_COL_M);
  page_left_dockmenu.bg.padding(0);                                 // กำหนดช่องไฟให้ เลเยอร์ bg ของ page เป็น 0
  page_left_dockmenu.scrl.padding(0);                               // กำหนดช่องไฟให้ เลเยอร์ scrl ของ page เป็น 0
  page_left_dockmenu.color(TFT_SILVER);
  page_left_dockmenu.bg.opa(150);
  page_left_dockmenu.scrl.opa(0);
  page_left_dockmenu.scrollbar(SCROLLBAR_AUTOHIDE, true, false);    // scrollbar ให้เป็นแบบ autohide โดย แนวนอนแสดง แนวตั้งไม่ให้แสดง
  page_left_dockmenu.position(-page_left_dockmenu.width(), 0);
  page_left_dockmenu.event_cb([](GWidget*w, event_t event){
    switch(event){
      case EVENT_PRESSING:
        timer_anim.del();
  ...
  // ให้ปุ่มที่สร้างจากรูปภาพ มีแม่คือ page_left_dockmenu พร้อมกำหนดคุณสมบัติการกด
  for(int i=0; i < SCREEN_NUM; i++){
    btns[i].parent(page_left_dockmenu);                             // ให้เป็นลูกของ page_left_dockmenu
...
void anim_page_left_dockmenu_hide(){
  page_left_dockmenu.toForeground();
  anim_page_left_dockmenu.init(page_left_dockmenu, ANIM_X, 0, -page_left_dockmenu.width(),1000);
  anim_page_left_dockmenu.path_type(ANIM_PATH_EASE_IN);
  anim_page_left_dockmenu.create();
}

void anim_page_left_dockmenu_show(){

  page_left_dockmenu.toForeground();
  anim_page_left_dockmenu.init(page_left_dockmenu, ANIM_X, -page_left_dockmenu.width(), 0,1000);
  anim_page_left_dockmenu.path_type(ANIM_PATH_EASE_IN);
  anim_page_left_dockmenu.ready_cb([](GAnimation*a){    
    timer_anim.setOnce(3000L,[](){
      if( page_left_dockmenu.posX() == 0) {
        anim_page_left_dockmenu_hide();
      }
...
void page_left_dockmenu_init_show_or_hide(){
  if( page_left_dockmenu.posX() == -page_left_dockmenu.width() ) {
    anim_page_left_dockmenu_show();
  }
  else 
