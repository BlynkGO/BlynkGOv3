/********************************************************************
 * ตัวอย่าง การสร้าง ปุ่มสวิทซ์ ด้วย GSwitch3D
 * ที่สามารถควบคุมได้จาก TFT และ แอพ Blynk
 * --------------------------------------------
 * ที่แอพ Blynk ให้สร้าง Widget Button สีแดง
...
GSwitch3D sw3d(TFT_RED);
// GSwitch3D sw3d(TFT_RED, SW3D_TYPE_RECT, V1);  // แบบกำหนด Virtual Pin ตั้งแต่ประกาศ
...
void setup() {
  ...
  //sw3d.sw3d_type(SW3D_TYPE_ROUND); // ปุ่มแบบวงกลม ไม่กำหนดจะเป็นปุ่มแบบเหลี่ยม

  /* หากมีการเปลี่ยนแปลง sw3d ไม่ว่าจากแอพ blynk หรือ กดจอ TFT
     จะมีการเปิด/ปิดไฟ ที่ ขา gpio ที่กำหนด 
     และมีการ update กันระหว่างแอพ blynk อัตโนมัติ
     ด้วย virtual pin ที่กำหนด */
  sw3d.gpio(39);          // กำหนดขา gpio สำหรับต่อกับ relay
  sw3d.virtualPin(V1);    // กำหนด virtual pin สำหรับ blynk

  /* เมื่อมีการเปลี่ยนแปลง sw3d ไม่ว่าจากแอพ blynk หรือ กดจอ TFT
     เพื่อ จัดการบางอย่างที่ต้องการ เพิ่มเติมเอง */
  sw3d.onValueChanged([](GWidget* widget){ 
    Serial.print("Changed! : ");
    Serial.println(sw3d.state());
  });
...
BLYNK_WRITE(V1){    
  sw3d.state(param.asInt());  // ให้ทำการ update สถานะ ของ sw3d ด้วย (ซึ่งจะอัพเดตขา gpio ด้วยในตัวอัตโนมัติ)
}
