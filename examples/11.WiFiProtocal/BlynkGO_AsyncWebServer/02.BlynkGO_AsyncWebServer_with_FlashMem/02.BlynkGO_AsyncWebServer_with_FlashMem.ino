 ...
 * โดยมีการ โหลด และบันทึก ค่าลง FlashMem ของ ESP32 ได้
 *  
 * ก่อนการ compile
...
GContainer con_inputs;
  GLabel lb_input_str   (con_inputs);
  GLabel lb_input_int   (con_inputs);
  GLabel lb_input_float (con_inputs);
...
void setup() {
  ...
  con_inputs.font(prasanmit_25, TFT_BLACK); // จัดฟอนต์ให้ ลูกๆ เป็นฟอนต์ และสีตามต้องการ  
    // โหลดข้อมูลจาก flashMem ให้ วิตเจ็ตข้อความ
    lb_input_str    = (BlynkGO.flashMem_exists("INPUT_STR"))?   BlynkGO.flashMem_String("INPUT_STR")  : "INPUT1";
    lb_input_int    = (BlynkGO.flashMem_exists("INPUT_INT"))?   BlynkGO.flashMem_String("INPUT_INT")  : "INPUT2";
    lb_input_float  = (BlynkGO.flashMem_exists("INPUT_FLOAT"))? BlynkGO.flashMem_String("INPUT_FLOAT"): "INPUT3";
