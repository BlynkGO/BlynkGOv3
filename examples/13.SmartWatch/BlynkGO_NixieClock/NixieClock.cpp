...
namespace NixieClock {
  ...
  void set_num(GImage *img, uint8_t num) {
    set_src(img[0], (uint8_t) (num / 10));
  ...
  void setTime(struct tm &tm){
    ...
    if( prev_tm.tm_hour != tm.tm_hour)  set_num( nixie_h, tm.tm_hour);
    if( prev_tm.tm_min  != tm.tm_min)   set_num( nixie_m, tm.tm_min);
    if( prev_tm.tm_sec  != tm.tm_sec)   set_num( nixie_s, tm.tm_sec);
  ...
  //  int nixie_h = nixie_h[0].height();
    ...
    obj_nixie_clock.dragable(true);
    obj_nixie_clock.corner_radius(10);
