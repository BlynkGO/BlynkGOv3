...
public:
  ...
  // TridentTD
  // trigger once on a given day and time
  AlarmID_t alarmOnce(const timeDayOfWeek_t DOW, time_t sec_of_day, OnTick_t onTickHandler) {
  ...
  // TridentTD
  // trigger weekly at a specific day and time 
  AlarmID_t alarmRepeat(const timeDayOfWeek_t DOW, time_t sec_of_day, OnTick_t onTickHandler) {
    time_t value = (DOW-1) * SECS_PER_DAY + sec_of_day;
    if (value <= 0) return dtINVALID_ALARM_ID;
    return create(value, onTickHandler, false, dtWeeklyAlarm);
  }
