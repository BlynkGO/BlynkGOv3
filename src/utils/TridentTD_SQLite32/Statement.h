...
// #include <WString.h>    // TridentTD
#include "Exception.h"

#include <string>
// #include <map>  // TridentTD ...hidden
...
public:
     ...
     * Exception is thrown in case of error, then the Statement object is NOT constructed.
     */
    Statement(Database& aDatabase, const String& aQuery);  // TridentTD
    ...
    void bind(const int aIndex, const String        aValue);   // TridentTD
    /**
    ...
    void bind_zeroblob(const int aIndex, const int aSize);
    /**
     * @brief Bind a string value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1).
     *
     ...
     */
    bool executeStep();
    bool available()                { return executeStep(); }  // TridentTD 
    /**
...
private:
    ...
    // typedef std::map<std::string, int> TColumnNames;     // TridentTD ...hidden
...
private:
    ...
    // mutable TColumnNames    mColumnNames;   //!< Map of columns index by name (mutable so getColumnIndex can be const) // TridentTD ...hidden
