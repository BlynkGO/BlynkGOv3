...
class Database
{
    ...
    friend class Blob;      // Give Blob constructor access to the mpSQLite Connection Handle //TridentTD
public:
    /**
    ...
    Database(const char* apFilename,
             const int   aFlags         = SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE, //SQLite::OPEN_READONLY,
    ...
    // TridentTD
    inline int exec(const String& aQueries)         {  return exec(aQueries.c_str());  }
    ...
    //TridentTD
    inline Column execAndGet(String  apQuery)       { return this->execAndGet(apQuery.c_str()); }

    // TridentTD
    int32_t getTableRowCount(String table_name);    // TridentTD
    int32_t getRowCount(String query);              // TridentTD : query example  "SELECT COUNT(*) FROM table;"
    ...
    // // TridentTD
    // void query(String aQuery);  // TridentTD สำหรับ query แบบ SELECT *..  เมื่อมี ข้อมูลคืนกลับมาจำนวนมาก
    // bool available();           // TridentTD ใช้ร่วมกับ API query(..) ว่าข้อมูลส่งมา ยังมีอยู่หรือไม่
    ...
    // TridentTD
    inline bool tableExists(const String& aTableName)
    {
        return tableExists(aTableName.c_str());
    }
...
// public: //TridentTD
    ...
    //TridentTD
    String file_path;
    // Statement *_query = NULL;     // pointer to db's query  // TridentTD
};
