...
#ifdef ESP32
#include "../BlynkGO_AsyncTCP/AsyncTCP.h"
...
class AsyncMqttClient {
 public:
  ...
  inline AsyncMqttClient& setServer(IPAddress ip, uint16_t port, const char* username, const char* password = nullptr) {
    this->setCredentials(username, password);
    this->setServer(ip, port);
  }

  inline AsyncMqttClient& setServer(const char* host, uint16_t port, const char* username, const char* password = nullptr) {
    this->setCredentials(username, password);
  ...
  inline AsyncMqttClient& onConnected(AsyncMqttClientInternals::OnConnectUserCallback callback)       { return onConnect(callback); }
  inline AsyncMqttClient& onDisconnected(AsyncMqttClientInternals::OnDisconnectUserCallback callback) { return onDisconnect(callback); }
  inline AsyncMqttClient& onSubscribed(AsyncMqttClientInternals::OnSubscribeUserCallback callback)    { return onSubscribe(callback); }
  inline AsyncMqttClient& onUnsubscribed(AsyncMqttClientInternals::OnUnsubscribeUserCallback callback){ return onUnsubscribe(callback); }
  inline AsyncMqttClient& onPublished(AsyncMqttClientInternals::OnPublishUserCallback callback)       { return onPublish(callback); }

  bool connected() const;
  void connect();
  void disconnect(bool force = false);
  uint16_t subscribe(const char* topic, uint8_t qos=QOS0);
  uint16_t unsubscribe(const char* topic);
  uint16_t publish(const char* topic, uint8_t qos, bool retain, const char* payload = nullptr, size_t length = 0, bool dup = false, uint16_t message_id = 0);

  inline uint16_t publish(String topic, String payload, uint8_t qos = QOS0, bool retain= false) { return publish(topic.c_str(), qos, retain, payload.c_str() ); }
  inline uint16_t publish(String topic, int payload_num, uint8_t qos = QOS0, bool retain= false) { return publish(topic.c_str(), String(payload_num)); }
  inline uint16_t clearRetain(String topic, uint8_t qos = QOS0)  { return publish(topic.c_str(), qos, true, NULL, 0); }
  inline uint16_t clearTopic(String topic, uint8_t qos = QOS0)   { return clearRetain(topic, qos); }
  inline uint16_t subscribe(String topic, uint8_t qos=QOS0)      { return subscribe(topic.c_str(), qos); }
  inline uint16_t unsubscribe(String topic)                      { return unsubscribe(topic.c_str()); }
