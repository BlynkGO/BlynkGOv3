...
//----------------------------------------------------------------------------------------------------------------------
typedef void(*fn_audio_handler_t)(void);  // TridentTD
...
public:
    ...
    // TridentTD
    inline bool play(fs::FS &fs, const char* path)          { return this->connecttoFS(fs, path);   }
    inline bool play(const char* path)                      { return this->connecttoSD(path);       }
    inline bool play_host(const char* host, const char* user = "", const char* pwd = "") { return this->connecttohost(host, user, pwd); }

    inline bool play(fs::FS &fs, String path)               { return this->connecttoFS(fs, path.c_str());   }
    inline bool play(String path)                           { return this->connecttoSD(path.c_str());       }
    inline bool play_host(String host, String user = "", String pwd = "") { return this->connecttohost(host.c_str(), user.c_str(), pwd.c_str()); }

    inline bool isPlaying()                                 { return this->isRunning();             }
    inline void stop()                                      { stopSong(); }
    inline void volume(uint8_t vol)                         { setVolume(vol); }
    inline uint8_t volume()                                 { return getVolume(); }
    inline uint32_t getAudioLeftTime()						{ uint32_t total = getAudioFileDuration() ; return  (total)? total - getAudioCurrentTime() : 0; }

    // TridentTD
    inline void onStop(fn_audio_handler_t fn)               { fn_on_stop = fn;  }
    inline void onPause(fn_audio_handler_t fn)              { fn_on_pause = fn; }
    inline void onPlay(fn_audio_handler_t fn)               { fn_on_play = fn;  }
    inline void onFinished(fn_audio_handler_t fn)           { fn_on_finished = fn;  }

    fn_audio_handler_t fn_on_stop=NULL;
    fn_audio_handler_t fn_on_play=NULL;
    fn_audio_handler_t fn_on_pause=NULL;
    fn_audio_handler_t fn_on_finished=NULL;
