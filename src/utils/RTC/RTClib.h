...
#include <Arduino.h>
#include "../TimeAlarms/TimeLib.h"    // BlynkGO
...
#endif // #if BLYNKGO_USE_RTC_DS323X

#if BLYNKGO_USE_RTC_PCF8523
...
#endif //#if BLYNKGO_USE_RTC_PCF8523

#if BLYNKGO_USE_RTC_PCF8563
...
public:
  ...
  */
  ...
  inline  uint8_t wday() const    { return dayOfTheWeek(); }
  ...
  inline uint32_t timestamp(void) const           { return unixtime(); }  // BlynkGO
  ...
  */
  ...
  String timestamp_str(timestampOpt opt = TIMESTAMP_FULL) const;   // BlynkGO เดิมคือ timestamp(...)
...
public:
  ...
  DateTime now();
  void   set(time_t timestamp); // BlynkGO
  void   set(uint8_t day, uint8_t month, uint16_t year, uint8_t hour, uint8_t minute, uint8_t second);  // BlynkGO
  inline uint8_t day()          { return now().day();       }       // BlynkGO
  inline uint8_t month()        { return now().month();     }
  inline uint16_t year()        { return now().year();      }
  inline uint16_t th_year()     { return year()+543;        }
  inline uint8_t wday()         { return now().wday();      }
  inline uint8_t hour()         { return now().hour();      }
  inline uint8_t minute()       { return now().minute();    }
  inline uint8_t second()       { return now().second();    }
  inline uint32_t timestamp()   { return now().timestamp(); }       // BlynkGO
...
#endif // BLYNKGO_USE_RTC_DS323X || BLYNKGO_USE_RTC_DS1307 || BLYNKGO_USE_RTC_PCF8523 || BLYNKGO_USE_RTC_PCF8563
#endif // _RTCLIB_H_
