...
#if defined ( ARDUINO )
    ledcDetachPin (_cfg.pin_bl);   // BlynkGO เพิ่มมาเพื่อล้างเดิมออกไปก่อน
    ledcSetup(_cfg.pwm_channel, _cfg.freq, 8);
    ledcAttachPin(_cfg.pin_bl, _cfg.pwm_channel);
    // Serial.printf("[Light PWM] init channel %d freq %d pin %d\n", _cfg.pwm_channel, _cfg.freq, _cfg.pin_bl);
...
#if defined ( ARDUINO )
      // Serial.printf("[::Light PWM] setBrightness: %d\n", brightness);
