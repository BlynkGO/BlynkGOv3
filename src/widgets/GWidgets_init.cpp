...
static GRect   *rect_blocker= NULL;
...
static void blynkgo_unregisted_info(){
  ...
  if( rect_blocker == NULL) {
    rect_blocker  = new_GObject();
    obj_message = new_GObject(rect_blocker);
    ...
    rect_blocker->toForeground();
  }else{
    rect_blocker->hidden(false);
    rect_blocker->toForeground();
  }
}
